import LocalFont from "next/font/local";

export const AyatNumberStyle12Font = LocalFont({
  src: "../assets/fonts/ayat-number-style-12.ttf",
});

export const IndopakFont = LocalFont({
  src: "../assets/fonts/indopak.ttf",
  variable: "--font-indopak"
});

export const UthmaniHafsFont = LocalFont({
  src: "../assets/fonts/uthmanic-hafs.ttf",
});

export const KFGQPCHafsUthmaniFont = LocalFont({
  src: "../assets/fonts/KfgqpcHafsUthmanicScriptRegular-nARZ1.ttf",
});

export const BazziV7 = LocalFont({
  src: "../assets/fonts/bazzi-v7.ttf",
});

export const HafsUthmanicV14 = LocalFont({
  src: "../assets/fonts/hafs-uthmanic-v14.ttf",
});

export const ARABIC_FONT = HafsUthmanicV14